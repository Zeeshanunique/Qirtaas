...
const uploadLargeFile = async (file: File, onProgress?: (progress: number) => void): Promise<string> => {
  try {
    ...
    const response = await axios.post('/api/upload-drive', formData, {
...
export default function SubmitPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const manuscriptUrl = await uploadLargeFile(manuscriptFile, (progress) => {
        setUploadProgress((prev) => ({ ...prev, manuscript: progress }))
        console.log(`Manuscript upload progress: ${progress}%`)
      })

      const coverUrl = await uploadLargeFile(coverFile, (progress) => {
      ...
      resetProgress() // Reset progress after successful submission
    } catch (error: any) {
      console.error('Submission error:', error)
      setSubmitStatus('error')
      setErrorMessage(error.message || 'An error occurred while submitting your work')
      resetProgress() // Also reset progress on error
    } finally {
